...
def exec_action(action, ask=False):
    if isinstance(action["function"]["arguments"], str):
        try:
            action["function"]["arguments"] = json.loads(
                action["function"]["arguments"]
            )
        except Exception as e:
            errmsg = (
                f"Error parsing json from GPT: {e} - arguments={action['function']}"
            )
            print(errmsg)
            return {"id": action["id"], "output": errmsg}
    arguments = action["function"]["arguments"]
    try:
        runaction = ACTIONS[action["function"]["name"]]
    except Exception as e:
        print(f"Error finding function: {e}")
        return {"id": action["id"], "output": f"Error finding function: {e}"}
    try:
        print("debug")
        print(arguments)
        runaction.prompt(**arguments)
    except Exception as e:
        return {"id": action["id"], "output": f"Error running command: {e}"}
    if ask:
        answer = input("Run? [Y/n] ").lower()
        if answer == "n":
            return {"id": action["id"], "output": ""}
    try:
        return {"id": action["id"], "output": runaction.run(**arguments)}
    except Exception as e:
...
def createFile(path: str, content: str):
    ...
    action.prompt(path, content)
    answer = input("Run? [Y/n] ").lower()
    if answer == "n":
        return {"error": "rejected by user"}
    result = action.run(path, content)
    return result


functions = [exec, createFile]
